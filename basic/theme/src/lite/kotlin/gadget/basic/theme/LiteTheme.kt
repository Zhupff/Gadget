...
internal object LiteTheme : LiveData<Theme>(), Theme, Loggable {
    ...
    override fun getColor(r: Theme.Resource): Int = when (r.id) {
        R.color.ThemePrimary    -> if (isLightMode) 0xFF445E91.toInt() else 0xFFADC6FF.toInt()
        R.color.ThemeOnPrimary  -> if (isLightMode) 0xFFFFFFFF.toInt() else 0xFF102F60.toInt()
        R.color.ThemeBackground -> if (isLightMode) 0xFFFFFFFF.toInt() else 0xFF0C0E13.toInt()
        R.color.ThemeForeground -> if (isLightMode) 0xFF1A1B20.toInt() else 0xFFE2E2E9.toInt()
        R.color.ThemeSurface    -> if (isLightMode) 0xFFEDEDF4.toInt() else 0xFF1E1F25.toInt()
        R.color.ThemeOutline    -> if (isLightMode) 0xFF74777F.toInt() else 0xFF8E9099.toInt()
        R.color.ThemeError      -> if (isLightMode) 0xFFBA1A1A.toInt() else 0xFFFFB4AB.toInt()
        R.color.ThemeOnError    -> if (isLightMode) 0xFFFFFFFF.toInt() else 0xFF690005.toInt()
        else -> {
