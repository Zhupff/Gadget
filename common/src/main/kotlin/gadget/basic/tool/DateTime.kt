...
class DateTime @JvmOverloads constructor(
    ...
    companion object {
         ...
         */
        @JvmStatic
        fun getSpecificTimestampInDay(timestamp: Long, offset: Long): Long {
            val zoneOffset = INSTANCE.formater.timeZone.rawOffset
            val dayOffset = timestamp % ONE_DAY
            return if (dayOffset <= zoneOffset) {
                timestamp / ONE_DAY * ONE_DAY - zoneOffset + offset
            } else if (ONE_DAY - dayOffset > zoneOffset) {
                timestamp / ONE_DAY * ONE_DAY - zoneOffset + offset
            } else {
                timestamp / ONE_DAY * ONE_DAY + ONE_DAY - zoneOffset + offset
            }
        }
         ...
         */
        @JvmStatic
        fun getDayGaps(timestamp1: Long, timestamp2: Long): Int {
            return ((getSpecificTimestampInDay(timestamp2, 0L) - getSpecificTimestampInDay(timestamp1, 0L)) / ONE_DAY).toInt()
        }
    }
